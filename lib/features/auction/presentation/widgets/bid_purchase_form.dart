...
import 'package:bidex/common/widgets/modal_form/dropdown.dart';
...
class _BidPurchaseFormState extends State<BidPurchaseForm> {
    ...
    return Flexible(
      child: listener(
        child: TopSection(
          flex: false,
          children: [
            const ttl.Title(title: 'Buy Bid'),
            const SizedBox(height: 8),
            const Heading(heading: 'Buy extra bid for 5GHS per bid'),
  ...
  Widget listener({required Widget child}) {
    return BlocListener<AuthBloc, AuthState>(
      listener: (context, state) {
        ...
        //   showFormDialog(context, const BidPurchaseForm());
        // }
      },
      child: child,
    );
